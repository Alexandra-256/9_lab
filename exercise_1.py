...
# Пример 1: Рациональная (несократимая) дробь представляется парой целых чисел (а, b),
# где а - числитель, b - знаменатель. Создать класс Rational для работы с рациональными дробями.
# Обязательно должны быть реализованы операции:
# сложения add, (a, b) + (c, d) = (ad + be, bd);
# вычитания sub, (a, b) - (c, d) = (ad - be, bd);
# умножения mul, (a, b) х (с, d) = (ac, bd);
